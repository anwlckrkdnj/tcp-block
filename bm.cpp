...
{
	BmCtx *tmp = (BmCtx*)malloc(sizeof(BmCtx));

	/* Prepare bad chars */
	PreBmBc(needle, needle_len, tmp->bmBc);

	tmp->bmGs = (uint16_t*)malloc(sizeof(uint16_t) * (needle_len + 1));

	/* Prepare good Suffixes */
	PreBmGs(needle, needle_len, tmp->bmGs);
